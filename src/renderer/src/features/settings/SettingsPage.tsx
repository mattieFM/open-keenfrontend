...
export function SettingsPage(): JSX.Element {
  ...
  const save = async () => {
    ...
    try {
      ...
      await updateWorkspace(workspace.id, { analyticsBaseUrl, dashboardBaseUrl, dashboardServiceEnabled: dashboardEnabled, organizationId: organizationId.trim() || undefined, preferences: { ...workspace.preferences, defaultTimezone: timezone, queryConcurrency: Math.max(1, Math.min(10, concurrency)), dashboardPersistence } });
...
function CredentialEditor({ existing, onClose, onSaved }: { existing?: CredentialMeta; onClose(): void; onSaved(): void }): JSX.Element {
  ...
  const [label, setLabel] = useState(existing?.label ?? 'Restricted access key'); const [type, setType] = useState<CredentialType>(existing?.type ?? 'access'); const [value, setValue] = useState(''); const [mode, setMode] = useState<StorageMode>(existing?.storageMode ?? 'memory'); const [passphrase, setPassphrase] = useState(''); const [error, setError] = useState('');
  const save = async () => {
    if (!workspace) return;
    try {
