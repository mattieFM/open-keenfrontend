...
export type WorkspaceState = {
  ...
  setCapability(id: string, operation: Operation, state: CapabilityState): Promise<void>;
};
...
export const useWorkspaceStore = create<WorkspaceState>((set, get) => ({
  ...
  async updateWorkspace(id, patch) {
    ...
    const updated: WorkspaceRecord = { ...current, ...patch, id, updatedAt: new Date().toISOString() };
    await db.workspaces.put(updated);
    set((state) => ({ workspaces: state.workspaces.map((item) => item.id === id ? updated : item) }));
  },

  async deleteWorkspace(id) {
    await db.transaction('rw', [db.workspaces, db.secrets, db.queryDrafts, db.knownSavedQueries, db.dashboards, db.audits], async () => {
    ...
    set((state) => {
      ...
      return { workspaces, runtimeModes, activeWorkspaceId: state.activeWorkspaceId === id ? workspaces[0]?.id : state.activeWorkspaceId };
    });
  },
