...
describe('visual dashboard query builder', () => {
  it('provides guided analysis, timeframe, filter, grouping, and funnel controls without a JSON editor', () => {
    ...
    expect(screen.getByLabelText('Event stream')).toHaveValue('slack_stream');
    ...
    fireEvent.change(screen.getByLabelText('Analysis'), { target: { value: 'funnel' } });
    expect(screen.getByText('Funnel steps')).toBeInTheDocument();
    expect(screen.getAllByLabelText(/Event stream/i).length).toBeGreaterThanOrEqual(2);
