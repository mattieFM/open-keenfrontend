...
try {
  ...
  await cp(root, validatorFixture, { recursive: true });
  ...
  const installPlan = runNode('scripts/ci-install.mjs', ['--dry-run']);
  expectSuccess(installPlan, 'dependency installation dry run');
  assert.match(output(installPlan), /Bootstrapping npm lockfile/u);
  assert.match(output(installPlan), /Dry run:/u);

  const productionInstallWithoutLock = runNode(
  ...
  console.log('Release pipeline self-test PASSED (source-tree guard, Dependabot-safe full-SHA validation, tag/channel and signing guards, collector, 8 packages, 4 manifests, checksums, provenance, manifest identity, tamper and unexpected-input rejection).');
} finally {
